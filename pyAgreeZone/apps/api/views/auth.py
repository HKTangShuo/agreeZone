...
class MessageView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if settings.SMS_ENABLE:
            ...
            random_code = settings.SMS_RANDOM_CODE

        conn = get_redis_connection()
        conn.set(phone, random_code, ex=settings.SMS_EXPIRED_TIME)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if settings.SMS_ENABLE:
            ...
            if not user_object:
                ...
                models.UserInfo.objects.filter(telephone=phone).update(**{**ser.validated_data, **result_dict},
                                                                       token=token)
